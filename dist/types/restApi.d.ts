...
type restValue = typeof currentAuthUser | typeof currentTimestamp | `${TOpStr | TOpBase}:${string}` | string | "" | "null" | number | null | boolean | undefined;
type TRegLike = "~" | "~~";
type TBoolNegative = "" | "!";
type TCase = "" | "*";
type restGetType<Fields extends string = string> = {
    [key in Fields as key extends Fields ? Fields | `${Fields}@${number}` | TqEXISTS | `${TqEXISTS}@${number}` : never]?: restValue | `@@:${key}` | `@@${TOpBase}:${key}` | Array<restValue | restGetType<Fields> | {
        [key in Fields as key extends Fields ? Fields : string]?: restValue;
    }> | IRestGet<Fields> | undefined;
};
