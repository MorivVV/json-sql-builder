...
export class WhereFilter extends BasicQuery {
  ...
  whereAnd(object: { [x: string]: string | IRestGet | number | boolean | null }) {
    const query: string[] = [];
    for (const key in object) {
      if (Object.prototype.hasOwnProperty.call(object, key)) {
        ...
        if (Array.isArray(element)) {
          ...
          const parseWhereValue = this.changeParameterValue(this.parseParameter(element as string | number | boolean));
  ...
  whereOr(array: { [x: string]: string | IRestGet | number | boolean | null }[]) {
    const query: string[] = [];
